...
Max_terms_list *mobius_find_max_terms(int max_term_limit, size_t dimension_limit, const Cipher_info * const cipher_info) {
    ...
    max_terms_list->max_terms = malloc(0);
    max_terms_list->max_term_count = 0;
    uint64_t zeroed_key[2] = {0, 0};
    while (max_terms_list->max_term_count < max_term_limit && dimensions[dimension_limit - 1] < cipher_info->iv_size + dimension_limit) {
        ...
        Max_terms_list* cube_max_terms = mobius_construct_max_terms(zeroed_key_poly, dimensions, max_terms_list->max_term_count, cipher_info);
...
int *mobius_is_super_poly_linear(int *zeroed_key_super_poly, int *cube_axes, int cube_dimension, const Cipher_info * const cipher_info) {
    ...
    for (int key_index_1 = 0; key_index_1 < number_of_keys - 1; key_index_1++) {
