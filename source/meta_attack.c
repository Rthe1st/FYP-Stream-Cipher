...
int main(int argc, char **argv) {
    ...
    cipher_info = grain_info();
    printf("grain guess\n");
    find_max_terms_for_inits_random(grain_random_fp, 32-8, 256, 10, cipher_info, 20, 10);
    free(cipher_info);
    //begin the lame
    cipher_info = grain_info();
    //no h
    ...
    printf("grain no_h\n");
    find_max_terms_for_inits(grain_no_h_fp, 31-8, 256, 10, cipher_info, 10);
    ...
    printf("grain no_l\n");
    find_max_terms_for_inits(grain_no_l_fp, 32-8, 256, 10, cipher_info, 10);
    char grain_no_l_random_fp[37] = ".\\grain_no_l_guess_max_terms_000.txt";
    printf("grain no_l rand\n");
    find_max_terms_for_inits_random(grain_no_l_random_fp, 37-8, 256, 10, cipher_info, 20, 10);
    ...
    printf("grain no_nl\n");
    find_max_terms_for_inits(grain_no_nl_fp, 33-8, 256, 10, cipher_info, 10);
    char grain_no_nl_random_fp[38] = ".\\grain_no_nl_guess_max_terms_000.txt";
    printf("grain no_nl rand\n");
    find_max_terms_for_inits_random(grain_no_nl_random_fp, 38-8, 256, 10, cipher_info, 20, 10);
    //make nlfsr linear
    ...
    printf("grain no_mnl\n");
    find_max_terms_for_inits(grain_no_mnl_fp, 33-8, 256, 10, cipher_info, 10);
    char grain_no_mnl_random_fp[39] = ".\\grain_no_mnl_guess_max_terms_000.txt";
    printf("grain no_mnl rand\n");
    find_max_terms_for_inits_random(grain_no_mnl_random_fp, 38-8, 256, 10, cipher_info, 20, 10);
    //no init
    NO_H = 0; NO_LFSR_F = 0; NO_NLFSR_F = 0;
    MAKE_NLFSR_LINEAR = 0; NO_INIT_LFSR_F = 1; NO_INIT_NLFSR_F = 1;
    char grain_no_ni_fp[32] = ".\\grain_no_ni_max_terms_000.txt";
    printf("grain no_ni\n");
    find_max_terms_for_inits(grain_no_ni_fp, 33-8, 256, 10, cipher_info, 10);
    char grain_no_ni_random_fp[38] = ".\\grain_no_ni_guess_max_terms_000.txt";
    printf("grain no_ni rand\n");
    find_max_terms_for_inits_random(grain_no_ni_random_fp, 38-8, 256, 10, cipher_info, 20, 10);
    //no_h, make nlfsr linear
    NO_H = 0; NO_LFSR_F = 0; NO_NLFSR_F = 0;
    MAKE_NLFSR_LINEAR = 0; NO_INIT_LFSR_F = 1; NO_INIT_NLFSR_F = 1;
    char grain_no_ml_fp[32] = ".\\grain_no_ml_max_terms_000.txt";
    printf("grain no_ml\n");
    find_max_terms_for_inits(grain_no_ml_fp, 32-8, 256, 10, cipher_info, 10);
    char grain_no_ml_random_fp[38] = ".\\grain_no_ml_guess_max_terms_000.txt";
    printf("grain no_ml rand\n");
    find_max_terms_for_inits_random(grain_no_ml_random_fp, 38-8, 256, 10, cipher_info, 20, 10);
    free(cipher_info);
    time_t rawtime;
    struct tm * timeinfo;
    time ( &rawtime );
    ...
    plain_v_mobius_time(grain_info(), 1, 3, 10, grain_timing);
    return EXIT_SUCCESS;
};
